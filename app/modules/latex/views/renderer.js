define(['sandbox', 'd3', 'psgraph', 'expressions', 'interact'], function(sandbox, d3, psgraph, expressions, interact) {

    var Views = {};

    var PSTricksView = sandbox.mvc.View({
    ...
    var EnumerateView = sandbox.mvc.View({
    ...
    var BasicView = sandbox.mvc.View({
        tagName: 'pre',
        beforeRender: function() {
            $(this.el).html(this.options.content.lines.join('\n'));
        }
    ...
    Views.TEX = sandbox.mvc.View({
        ...
        beforeRender: function() {
            ...
            _.each(this.options.latex, function(element) {
                ...
                if (element.type.match(/math/)) {
                    var math = new MathView({
                        content: element
                    });
                    this.insertView(math);

                } else if (element.type.match(/pspicture/)) {

                    var pstricks = new PSTricksView({
                        content: element
                    });
                    ...
                    if (element.env.sliders && element.env.sliders.length) {
                        ...
                        var slidersView = new interact.SlidersView({
                            env: element.env,
                ...
                } else if (element.type.match(/verbatim/)) {

                    var raw = new BasicView({
                        content: element
                    });
                    this.insertView(raw);

                } else if (element.type.match(/enumerate/)) {

                    var enumerate = new EnumerateView({
                        content: element
                    });
                    this.insertView(enumerate);

                } else if (element.type.match(/nicebox/)) {

                    var well = new WellView({
                        content: element
                    });
                    this.insertView(well);

                } else {
                    //console.log('dont know what this is: ' + element.type);
                }
